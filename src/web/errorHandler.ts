...
const handleRequestErrors = function handleRequestErrors(
  ...
  if (error instanceof EntityNotFoundError) {
    logger.warn(error.message, { excludeFromBreadcrumb: false })
  ...
  if ((error as HttpError).code === 'EBADCSRFTOKEN') {
    logger.warn('Bad CSRF token received for request', { excludeFromBreadcrumb: false })
...
const handleDefault = function handleDefault(
  ...
  logger.warn('Unhandled error caught by middleware stack', { error, excludeFromBreadcrumb: false })
