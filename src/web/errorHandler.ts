...
import { EntityNotFoundError, RESTClientError, ValidationError, NotImplementedError } from '../lib/errors'
...
const handleRequestErrors = function handleRequestErrors(
  ...
  if (error instanceof NotImplementedError) {
    logger.warn(error.message)
    res.status(500).render('common/error', { message: error.message })
    return
  }
